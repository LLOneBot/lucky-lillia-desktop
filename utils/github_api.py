    ...
    for mirror in mirrors:
        try:
            ...
            if mirror == "https://github.com/":
                # 直连GitHub，使用API
                result = _get_release_from_api(repo, timeout)
            else:
                # 使用镜像，通过解析release页面获取信息
                result = _get_release_from_mirror(repo, mirror, timeout)
            ...
            logger.warning(f"镜像 {mirror} 连接失败: {e.reason}")
            continue  # 尝试下一个镜像
...
def _get_release_from_api(repo: str, timeout: int) -> Optional[Dict[str, Any]]:
...
def _get_release_from_mirror(repo: str, mirror: str, timeout: int) -> Optional[Dict[str, Any]]:
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as response:
            # 从重定向URL或页面内容中提取tag
            # GitHub的latest release会重定向到具体版本，如 /releases/tag/v1.0.0
